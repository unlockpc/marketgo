...
function initModals() {
    ...
    document.getElementById('nurtureAllConcurrency')?.addEventListener('change', (ev) => {
        ...
        ev.target.value = String(n); // 回写夹紧后的值
...
let nurtureAllProfileKeys = new Set();
...
function computeNurtureAllPlan() {
    const todo = [];
    const seen = new Set(); // 按账号 id 去重，防止同一账号被列两次
    let skipped = 0;
    for (const account of accounts) {
...
async function loadSettings() {
    ...
    if (concInput)
        concInput.value = String(getNurtureAllConcurrency());
